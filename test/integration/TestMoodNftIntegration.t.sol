...
contract TestMoodNftIntegration is Test{
    ...
    address USER = makeAddr("user_lmfao");
    ...
    function testViewTokenUri_Integration() public {
        vm.prank(USER);
        moodNft.mintNft();
        console.log(moodNft.tokenURI(0));
    }

    function test_MoodFlips() public{
        vm.startPrank(USER);
        moodNft.mintNft();

        assertEq(
            keccak256(abi.encodePacked(moodNft.getImageURI(0))),
        ...
        assertEq(
            keccak256(abi.encodePacked(moodNft.getImageURI(0))),
            keccak256(abi.encodePacked(SAD_SVG_URI)) // can also test tokenURI instead of imageURI
        );
